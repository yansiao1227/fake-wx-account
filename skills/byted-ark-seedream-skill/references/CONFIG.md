...
| 优先级 | 路径 | 说明 |
|-------|------|------|
| 1 | `ARK_SEEDREAM_SAVE_PATH` / `ARK_SAVE_PATH` | 显式环境变量（bash 会纠正指向 `~/cow/tmp` 的旧配置） |
| 2 | `{COW_PROJECT_ROOT}/tmp/seedream-images/` | 默认：项目根目录临时目录（非全局 `~/cow`） |
| 日期子目录 | `.../YYYY-MM-DD/` | 脚本按日归档 |

生成后须用 Agent 的 **`send` 工具** 发送 `local_path`，通道才会把图片交给用户。
