...
/**
 * Resolve CowAgent project root (repo / app install dir).
 * Prefer COW_PROJECT_ROOT injected by the bash tool so Seedream images land
 * under <project>/tmp/seedream-images instead of the global ~/cow workspace.
 */
function resolveProjectRoot() {
  const path = require('path');
  const fs = require('fs');
  const candidates = [
    process.env.COW_PROJECT_ROOT,
    process.env.AGENT_WORKSPACE,
    process.env.COW_WORKSPACE,
    process.env.COW_AGENT_WORKSPACE,
  ];
 ...
 * 智能获取默认保存路径
 * 优先级:
 *   1. ARK_SEEDREAM_SAVE_PATH / ARK_SAVE_PATH
 *   2. 项目根目录 tmp/seedream-images（非全局 ~/cow）
 */
function getDefaultSavePath() {
  const path = require('path');

  // 1. 优先使用环境变量配置（bash 会注入项目 tmp 路径，并纠正旧的 ~/cow 路径）
  if (process.env.ARK_SEEDREAM_SAVE_PATH) {
    return path.resolve(expandHome(process.env.ARK_SEEDREAM_SAVE_PATH));
  }
  if (process.env.ARK_SAVE_PATH) {
    return path.resolve(expandHome(process.env.ARK_SAVE_PATH));
  }

  // 2. 默认：<project>/tmp/seedream-images
  return path.join(resolveProjectRoot(), 'tmp', 'seedream-images');
}
