...
async function executeStep(stepName, stepFunction, context = {}) {
  ...
  try {
    ...
    if (context?.warnings) {
      context.warnings.push(createErrorMessage(stepName, error, ErrorLevel.WARNING));
    }
...
async function saveToFeishuCloud(notes, context) {
  ...
  const feishuDisabled = String(process.env.FEISHU_DISABLED || "").toLowerCase() === "true";
  if (!isValidConfig || feishuDisabled) {
    const warningMessage = createErrorMessage(
      "feishu-config",
      new Error(
        feishuDisabled
          ? "已跳过飞书归档（FEISHU_DISABLED=true）"
          : "飞书配置无效，已跳过飞书归档（可设置 FEISHU_APP_ID/FEISHU_APP_SECRET/FEISHU_BASE_FOLDER_TOKEN）",
      ),
      ErrorLevel.WARNING,
    );
    context.warnings.push(warningMessage);
...
async function saveNoteToFeishu(note, context) {
  ...
  const dateParts = resolveStorageDateParts(note.content.publishTime);

  // 创建或获取分类/年/月 目录
  const folderToken = await getOrCreateCategoryYearMonthFolder(category, dateParts);

  // 上传文件
  const uploadResult = await uploadMarkdownFile(note.markdown, filename, folderToken);

  return {
    path: `${LARK_CONFIG.cloudDriveSpaceName}/${category}/${dateParts.year}/${dateParts.month}/${filename}`,
    fileUrl: uploadResult.fileUrl,
    fileToken: uploadResult.fileToken,
  };
}

function resolveStorageDateParts(rawPublishTime) {
  const parsed = rawPublishTime ? new Date(rawPublishTime) : new Date();
...
async function getOrCreateChildFolder(parentFolderToken, folderName) {
  try {
...
// 获取或创建 分类/年/月 文件夹
async function getOrCreateCategoryYearMonthFolder(category, dateParts) {
  try {
    const categoryFolderToken = await getOrCreateChildFolder(LARK_CONFIG.baseFolderToken, category);
    const yearFolderToken = await getOrCreateChildFolder(categoryFolderToken, dateParts.year);
    return await getOrCreateChildFolder(yearFolderToken, dateParts.month);
  } catch (error) {
    throw new Error(`创建分类年月文件夹失败: ${error.message}`);
  }
}
...
async function main(input, context = {}) {
  ...
  context.traceId =
    context.traceId || `dog-${Date.now()}-${Math.random().toString(16).slice(2, 10)}`;
  ...
  try {
    ...
    // 步骤1：验证微信链接
    const urls = await executeStep("validate-links", () => validateWeChatLink(input), context);
