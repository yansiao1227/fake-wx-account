...
def _parse_channel_type(raw) -> list:
    ...
    Supports:
      - single string: "web"
      - comma-separated string: "web, wechat_desktop"
      - list: ["web", "weixin"]
    """
    ...
    try:
        ...
        if _channel_mgr is not None:
            try:
                ...
                logger.warning(f"[App] Channel shutdown failed: {exc}")
        try:
            conf().save_user_datas()
        except Exception as exc:
            logger.warning(f"[App] Failed to save user data during shutdown: {exc}")
