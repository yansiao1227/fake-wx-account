...
### Agent Plan (enterprise subscription)

When using a Volcengine **Agent Plan** key, point the base URL at the plan endpoint and keep the short model name:
...
| Env keys | `ARK_API_KEY` / `ARK_API_BASE` (or `AGENT_PLAN_API_KEY` as key fallback) |
| Compatible model | `doubao-seedream-5-0-lite` (alias `seedream-5.0-lite`) |

Agent Plan keys only authenticate against `/api/plan/v3`, not the standard `/api/v3`. The image-generation skill rewrites dated Seedream model IDs to the Plan-compatible short name automatically.
