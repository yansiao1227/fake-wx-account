...
| `type` / `transport` | Remote | Remote transport: `sse`, `http` / `streamable-http` (official samples often use `transport: "http"`) |
| `headers` | No | Extra HTTP headers (e.g. `Authorization`, `X-Agent-Plan-Key`); supports `${ENV_VAR}` placeholders |
...
### Volcengine Professional Dataset (Agent Plan MCP)

With an Agent Plan subscription, add:
...
Put the key in `~/.cow/.env` as `AGENT_PLAN_API_KEY` (or reuse `WEB_SEARCH_API_KEY` / Ark subscription key). After load, the MCP tool `dataPro_search` becomes available.
