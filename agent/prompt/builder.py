...
def _build_tooling_section(tools: List[Any], language: str) -> List[str]:
    ...
    if is_en:
        lines = [
            ...
            "- Before ending after any tool call, compare the original request with the evidence obtained. Empty, ambiguous, partial, or out-of-scope results require another tool call or one precise clarification — never fill gaps by guessing",
            "- A successful tool call is not the same as a completed task. Finish every requested verification, calculation, comparison, or follow-up action before answering",
        ...
        lines = [
            ...
            "- 每次工具调用后准备结束前，都要把原始请求与现有证据逐项对照。空结果、歧义结果、部分结果或超出指定范围的结果，都必须继续查证或只问一个精准的澄清问题，禁止靠猜测补齐",
            "- 工具执行成功不等于任务完成；用户要求的验证、计算、比较和后续动作必须全部完成后才能回答",
            "- 回复中涉及密钥、令牌等敏感信息必须脱敏",
            "- URL链接直接放在回复文本中即可，系统会自动处理和渲染。无需下载后使用send工具发送",
            "- 用户要求获取、查看、读取、查找或总结微信最近聊天记录时，必须调用 `wechat_history` 工具，并将用户要求的条数作为 `limit`（最多50）；不能只根据当前上下文直接回答。",
            "",
        ]
