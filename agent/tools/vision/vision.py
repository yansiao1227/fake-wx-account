...
class Vision(BaseTool):
    ...
    def _resolve_providers(self) -> List[VisionProvider]:
        ...
        if use_linkai:
            ...
            self._append_provider(
                fallback,
                lambda: self._build_legacy_custom_provider(user_model),
            )
    ...
    def _build_legacy_custom_provider(
        self, preferred_model: Optional[str] = None
    ) -> Optional[VisionProvider]:
        """Reuse the active legacy custom OpenAI-compatible provider for vision."""
        if conf().get("bot_type") != const.CUSTOM:
            return None
        api_key = str(conf().get("custom_api_key") or "").strip()
        api_base = str(conf().get("custom_api_base") or "").strip()
        model = str(preferred_model or conf().get("model") or "").strip()
