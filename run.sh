...
select_channel() {
    echo ""
    CHANNEL_KEYS=(web wechat_desktop weixin skip)
    local labels=() k sel
    for k in "${CHANNEL_KEYS[@]}"; do labels+=("$(channel_label "$k")"); done
    select_menu sel "$(t "选择微信接入方式" "Select WeChat Channel")" "${labels[@]}"
...
configure_channel() {
    case "$channel_choice" in
        ...
        weixin)
            CHANNEL_TYPE="weixin"
            ACCESS_INFO="$(t "微信官方接口已配置，请扫码登录" "Official Weixin configured. Scan QR code to login.")"
            ;;
        wechat_desktop)
