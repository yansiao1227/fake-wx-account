...
function Select-Channel {
    Write-Host ""
    $script:ChannelKeys = @("web", "wechat_desktop", "weixin", "skip")
    $options = @($script:ChannelKeys | ForEach-Object { Get-ChannelLabel $_ })
    $idx = Select-Menu -Title (T "选择微信接入方式" "Select WeChat Channel") -Options $options -Default 1
...
function Configure-Channel {
    ...
    switch ($script:ChannelChoice) {
        ...
        "weixin" {
            ...
            $script:AccessInfo = T "微信官方接口已配置，请扫码登录" "Official Weixin configured. Scan QR code to login."
        }
        "wechat_desktop" {
