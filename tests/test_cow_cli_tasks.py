...
def test_tasks_command_only_lists_tasks_owned_by_current_channel_and_receiver(monkeypatch):
    store = FakeTaskStore(
        [
            _task("mine", "user-1", "web"),
            _task("other-channel", "user-1", "weixin"),
            _task("other-user", "user-2", "web"),
        ]
    )
    monkeypatch.setattr(
    ...
    result = plugin._cmd_tasks(
        "", _event_context("web", "user-1", "session-1")
    )
...
def test_tasks_command_uses_session_identity_without_channel_context(monkeypatch):
    store = FakeTaskStore(
        [
            ...
            _task("notified", "chat-1", "weixin", notify_session_id="session-1"),
...
def test_tasks_command_formats_empty_state(monkeypatch):
    ...
    result = plugin._cmd_tasks(
        "", _event_context("wechat_desktop", "channel-1", "session-1")
    )
