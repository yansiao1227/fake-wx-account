...
class ChatChannel(Channel):
    ...
    def __init__(self):
        ...
        # which caused contexts from one channel to be consumed by another
        # channel's consume() thread, leading to errors
        ...
        if reply and reply.type:
            ...
            if not e_context.is_pass() and reply and reply.type:
                ...
                # 如果是图片回复但带有文本内容，先发文本再发图片
                    ...
                    time.sleep(0.3)
                    self._send(reply, context)
                # Send the transcript bubble before the synthesized voice reply.
