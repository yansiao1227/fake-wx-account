...
def _get_workspace_root() -> str:
    """Return the configured agent workspace as an absolute local path."""
    return os.path.abspath(expand_path(conf().get("agent_workspace", "~/cow")))
...
def _require_wechat_desktop_security():
    """Protect desktop-control data even when the general console has no password.

    A password-less console is safe only on a loopback listener.  In particular,
    explicitly binding to a LAN address or all interfaces must never expose the
    desktop-WeChat status or action endpoints.
    """
    _require_auth()
    configured_host = str(conf().get("web_host", "") or "").strip().lower()
    if not configured_host:
        configured_host = "127.0.0.1"
    configured_host = configured_host.strip("[]")
    if not _is_password_enabled() and configured_host not in {
        "127.0.0.1",
        "::1",
        "localhost",
    }:
            ...
            json.dumps(
                {
                    "status": "error",
                    "message": (
                        "Desktop WeChat control requires a loopback web_host "
                        "or a configured web_password"
                    ),
                }
            ),
        )
...
class WebChannel(ChatChannel):
    ...
    def startup(self):
        ...
        urls = (
            ...
            '/api/wechat-desktop/status', 'WechatDesktopStatusHandler',
            '/api/wechat-desktop/pause', 'WechatDesktopPauseHandler',
...
class ModelsHandler:
    ...
    def _handle_set_voice_reply_mode(self, data: dict) -> str:
        ...
        # reads, so all retained channels share the routing.
...
class ChannelsHandler:
    """API for managing the WeChat and desktop-WeChat channels."""
    ...
    CHANNEL_DEFS = OrderedDict([
        ...
        ("wechat_desktop", {
            "label": {"zh": "桌面微信接管", "en": "Desktop WeChat"},
            "icon": "fa-desktop",
            "color": "emerald",
            "fields": [],
        }),
    ...
    def _handle_connect(self, channel_name: str, updates: dict):
        ...
        def _do_start():
            try:
                ...
                # Wait for the old connection to release before reconnecting.
...
class WechatDesktopStatusHandler:
    def GET(self):
        _require_wechat_desktop_security()
        web.header('Content-Type', 'application/json; charset=utf-8')
        web.header('Cache-Control', 'no-store')
        try:
            from channel.wechat_desktop.service import get_wechat_desktop_service
...
class WechatDesktopPauseHandler:
    def POST(self):
        _require_wechat_desktop_security()
        web.header('Content-Type', 'application/json; charset=utf-8')
        try:
