...
// =====================================================================
let channelsData = [];
let wechatDesktopPollTimer = null;
...
function loadChannelsView() {
    ...
    fetch('/api/channels').then(r => r.json()).then(data => {
        ...
        if (channelsData.some(ch => ch.name === 'wechat_desktop' && ch.active)) {
            loadWechatDesktopPanel();
            stopWechatDesktopPoll();
            wechatDesktopPollTimer = setInterval(loadWechatDesktopPanel, 3000);
        } else {
            stopWechatDesktopPoll();
            const panel = document.getElementById('wechat-desktop-panel');
...
function stopWechatDesktopPoll() {
    if (wechatDesktopPollTimer) {
        clearInterval(wechatDesktopPollTimer);
        wechatDesktopPollTimer = null;
    }
}

function loadWechatDesktopPanel() {
    const panel = document.getElementById('wechat-desktop-panel');
    const statusEl = document.getElementById('wechat-desktop-status');
    if (!panel || !statusEl) return;
    fetch('/api/wechat-desktop/status').then(r => r.json()).then(statusData => {
        if (statusData.status !== 'success') return;
        panel.classList.remove('hidden');
        const s = statusData.desktop || {};
        const zh = currentLang === 'zh' || currentLang === 'zh-Hant';
            ...
            <div class="flex items-start justify-between gap-4">
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="font-semibold text-slate-800 dark:text-slate-100">${zh ? '桌面微信自动化' : 'Desktop WeChat automation'}</h3>
                        ${s.shadow_mode ? `<span class="text-xs px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">${zh ? '影子模式' : 'Shadow mode'}</span>` : ''}
                    </div>
                    <div class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500 dark:text-slate-400">
                        <span>${zh ? '状态' : 'Status'}: ${stateText}</span>
                    ...
                    ${s.last_error ? `<p class="mt-2 text-xs text-red-500">${escapeHtml(s.last_error)}</p>` : ''}
                </div>
                <button onclick="setWechatDesktopPaused(${paused ? 'false' : 'true'})"
                        class="px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer ${paused ? 'bg-emerald-500 text-white' : 'bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-300'}">
                    ${paused ? (zh ? '恢复接管' : 'Resume') : (zh ? '暂停接管' : 'Pause')}
                </button>
            </div>`;
    }).catch(() => {});
}

function setWechatDesktopPaused(paused) {
    fetch('/api/wechat-desktop/pause', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ paused }),
    }).then(() => loadWechatDesktopPanel());
}
...
navigateTo = function(viewId) {
    ...
    if (currentView === 'channels' && viewId !== 'channels') stopWechatDesktopPoll();
...
// CowAgent opens on operational status by default. Chat remains available
// from the sidebar and from explicit message/session actions.
navigateTo('logs');
...
function openTaskEditModal(task) {
    ...
    //   2. The receiver identity itself is channel-bound, so changing the channel
